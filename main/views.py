...
from main.models import Test, Question, CheckQuestion, CheckTest
from django.contrib.auth.decorators import login_required
from django.http import HttpResponse
from django.utils import timezone
from main.forms import TestForm, QuestionForm
...
def redy_to_test(request, test_id):
    test = get_object_or_404(Test, id=test_id)
    return render(request, 'redy_to_test.html', {'test': test})
    ...
    if (timezone.now()>=test.start_date and timezone.now()<=test.end_date) and attemps<test.maximum_attemps:
        ...
        if request.method == 'POST':
            ...
            for question in questions:
                ...
                CheckQuestion.objects.create(checktest=checktest, question=question, given_answer=given_answer, tru_answer=question.tru_answer)
                
            checktest.save()
            return redirect('checktest', checktest.id)
        context = {'test': test, 'question':questions}
